...
glm::vec4 Image::getPixelInterpolated(float x, float y, bool repeat) {
	...
	glm::vec4 top = glm::mix(getPixel(ix, iy), getPixel(ix2, iy), fx);
	glm::vec4 bottom = glm::mix(getPixel(ix, iy2), getPixel(ix2, iy2), fx); // lerp == glm::mix ?
...
glm::vec4 Image::getPixelInterpolatedHigh(float x, float y, bool repeat) {
	...
	int iy2 = iy < static_cast<int>(height) - 1 ? iy + 1 : 0;
	glm::vec4 top = glm::mix(getPixel(ix, iy), getPixel(ix2, iy), fx);
	glm::vec4 bottom = glm::mix(getPixel(ix, iy2), getPixel(ix2, iy2), fx);
	return glm::mix(top, bottom, fy);
};
...
void Texture::createCubemap(unsigned int width, unsigned int height, uint8_t** data, unsigned int format, unsigned int type, bool mipmaps, unsigned int internal_format)
{
	assert(width && height && "texture must have a size");
...
void Texture::uploadCubemap(unsigned int format, unsigned int type, bool mipmaps, uint8_t** data, unsigned int internal_format) {
