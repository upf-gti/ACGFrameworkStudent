...
Shader* Shader::getDefaultShader(std::string name)
{
	...
	vs = "#version 410 core in vec3 a_vertex; in vec3 a_normal; in vec2 a_uv; in vec4 a_color; \
	...
	if (name == "flat")
	{
		fs = "#version 410 core uniform vec4 u_color;\n\
            out vec4 FragColor;\n\
			void main() {\n\
				FragColor = u_color;\n\
			}";
	...
	{
		fs = "#version 410 core in vec4 v_color;\n\
	...
	{
		vs = "#version 410 core in vec3 a_vertex; \
			out vec2 v_uv;\n\
			void main()\n\
			{\n\
		...
		fs = "#version 410 core in vec2 v_uv;\n\
			uniform sampler2D u_texture;\n\
            out vec4 FragColor;\n\
			void main() {\n\
	...
	else if (name == "linear_depth")
	{
		...
		fs = "#version 410 core uniform vec2 u_camera_nearfar;\n\
	...
	else if (name == "quad" || name == "textured_quad") //draws a quad
	{
		...
		if (name == "textured_quad")
			...
			fs = "#version 410 core uniform vec4 u_color; out vec4 FragColor;\n\
			void main() {\n\
				FragColor = u_color;\n\
			}";
	}
